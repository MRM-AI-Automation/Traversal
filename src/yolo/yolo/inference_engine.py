...
class YOLOObjectDetectionNode(Node):
    def __init__(self):
       ...
       # self.imu_sub = self.create_subscription(ImuData, "/imu_data", self.imu_callback, 10)
        ...
        self.image_sub = self.create_subscription(Image, '/zed/zed_node/rgb/image_rect_color', self.img_callback, 10)
    ...
    def detect_objects(self, frame):
        if frame is not None:
            ...
            pred = self.model.predict(pil_frame,device='cuda',half=True,verbose=False)
            pred = pred[0]
